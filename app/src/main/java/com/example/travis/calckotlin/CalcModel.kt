...
class CalcModel : Serializable {
    ...
    fun handleButtonPressed(string: String) {
        when (string) {
            ...
            "1/x" -> {
                appendOnResult("(1/", ")", false, true)
            }
            "2sqrt" -> {
                appendOnResult("sqrt(", ")", false, true)
            }
            "3sqrt" -> {
            ...
            "x!" -> {
                appendOnResult("", "!", false, true)
            }
            "e" -> {
                appendOnResult("e", "", false, true)
            }
            "Pi" -> {
                appendOnExpression(Math.PI.toString())
            }
            "EE" -> {
    ...
    private fun appendOnResult(
        ...
        tvResultString = if (calculateThis) {
    ...
    private fun deleteLustOperation(ExpressionString: String): String {
        return if (ExpressionString.endsWith("+") || ExpressionString.endsWith("-") || ExpressionString.endsWith("/") || ExpressionString.endsWith(
                "*"
